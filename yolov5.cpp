...
int main(int argc, char** argv) {
    ...
    for (int f = 0; f < (int)file_names.size(); f++) {
        ...
        for (int b = 0; b < fcount; b++) {
            ...
            std::string filename=file_names[f - fcount + 1 + b];
            std::string pureName = filename.substr(0, filename.rfind(".")); // 去掉后缀后的前缀名称
            std::ofstream out_file(pureName+".txt");

            for (size_t j = 0; j < res.size(); j++) {
                cv::Rect r = get_rect(img, res[j].bbox);

                out_file<<res[j].class_id<<" "<<r.x<<" "<<r.y<<" "<<r.width<<" "<<r.height<<"\n";
