    ...
    for (int f = 0; f < (int)file_names.size(); f++) {
        ...
        for (int b = 0; b < fcount; b++) {
            ...
            for (int row = 0; row < INPUT_H; ++row) {
                uchar* uc_pixel = pr_img.data + row * pr_img.step;//以行来读取mat中的信息 将其赋值给static array
        ...
        doInference(*context, stream, buffers, data, prob, BATCH_SIZE);//进行推理 结果存储在prob中
        ...
        for (int b = 0; b < fcount; b++) {
            ...
            for (size_t j = 0; j < res.size(); j++) {
                cv::Rect r = get_rect(img, res[j].bbox); // 得到相对于原图的bbox信息
                std::cout<<"("<<r.x<<","<<r.y<<","<<r.width<<","<<r.height<<")"<<std::endl;//输出bbox
                cv::rectangle(img, r, cv::Scalar(0x27, 0xC1, 0x36), 2);// 根据计算的bbox 在图上绘制bbox
